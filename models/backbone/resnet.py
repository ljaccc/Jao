...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, dcn=None):
        ...
        self.bn1 = BatchNorm2d(planes)
        self.with_modulated_dcn = False
        #self.conv2_offset = nn.Conv2d(planes, planes , stride=stride, kernel_size=3,padding=1)
        # if not self.with_dcn:
        ...
        #     from torchvision.ops import DeformConv2d
        #     offset_channels = 18
        #     #self.conv2_offset = nn.Conv2d(planes, deformable_groups * offset_channels, stride=stride, kernel_size=3,=1)
    ...
    def forward(self, x):
        ...
        out = self.conv2(out)
        # else:
        #     offset = self.conv2_offset(outs)
        #     outs = self.conv2(outs, offset)
        out = self.bn2(out)
        out = self.relu(out)
...
def resnet50(pretrained=True, **kwargs):
    ...
    if pretrained:
        ...
        # TODO: 读取的是本地路径，需要在该目录下放置对应resnet50权重
